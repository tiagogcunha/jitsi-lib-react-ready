...
export class RemoteTrack extends React.Component {
    ...
    componentDidMount () {
        ...
        this.tracks = _.filter(window.jit.remoteTracks, (rt) => { return _.indexOf(trackIds, rt.id) !== -1 })
    ...
    componentDidUpdate (prevProps) {
        ...
        if (currentTrackIdText !== previousTrackIdText) {
            ...
            this.tracks = _.filter(window.jit.remoteTracks, { participantId: participantId })
            let videoTrack = _.find(this.tracks, { type: 'video' })
            let micTrack = _.find(this.tracks, { type: 'audio' })
            let newState = {}
            if (videoTrack) {
    ...
    onTrackStoppedEvent = (event) => {
        console.warn(`Track Stopped`)
    }

    onTrackAudioOutputChangedEvent = (deviceId) => {
        console.warn(`Track ${deviceId} audio output changed`)
    }
