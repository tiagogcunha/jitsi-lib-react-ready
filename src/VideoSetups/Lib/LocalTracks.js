...
export class LocalTracks extends React.Component {
      ...
      (tracks) => {
        ...
        this.setState(
          ...
          () => {
            ...
            if (activeRoomId && window.jit.activeRoom) {
              ...
              if (videoTrack) {
                window.jit.activeRoom.addTrack(videoTrack);
              }
              if (micTrack) {
                window.jit.activeRoom.addTrack(micTrack);
              }
            }
  ...
  onTrackVideoOutputChangedEvent = (deviceId) => {
    console.log(`Track ${deviceId} video output changed`);
  };
  ...
  componentDidUpdate(prevProps, prevState) {
    if (this.state.initialized) {
      ...
      if (activeRoomId.HasChanged) {
        if (activeRoomId.Current && window.jit.activeRoom) {
    ...
    return (
      <div className="local_track">
        ...
        <div className="local_track_body">
          <video autoPlay="1" ref={this.videoRef} playsInline />
        </div>
        {/* <div>
